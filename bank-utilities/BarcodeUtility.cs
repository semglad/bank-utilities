...
namespace Ekoodi.Utilities.Finance
{
    public class BarcodeUtility
    {
        ...
        public static string CreateBarcode(string ibanNumber, double invoiceSum, string referenceNumber, DateTime dueDate)
        {
            ...
            string barcode = ibanNumber.Remove(0, 2) + NumberUtility.AddZeroPadding(Math.Round(invoiceSum, 2).ToString("F2").Replace(",", ""), 0, 8);

            // Add reference number to the barcode
            if (referenceNumber.Substring(0, 2) == "RF")
                barcode = "5" + barcode + NumberUtility.AddZeroPadding(referenceNumber.Remove(0, 2), 2, 23);
            else
                barcode = "4" + barcode + "000" + NumberUtility.AddZeroPadding(referenceNumber, 0, 20);
